...
<nav class="navbar navbar-dark fixed-top bg-danger flex-md-nowrap p-0 shadow"><a class="navbar-brand col-sm-3 col-md-2 mr-0" href="dashboard.php">VIT PLACEMENT</a></nav>
    ...
    <ul class="nav flex-column">
		...
		<p class="bg-dark text-white p-2">Placement_offers</p>
	    <?php
	    $sql = "SELECT * FROM placement_offers";
		...
		<table class="table">
			...
			<thead>
			<tr>
			<th scope="col">placement offer id</th>
			<th scope="col">reg_no</th>
			<th scope="col">comp_id</th>
			<th scope="col">offer_type</th>
			<th scope="col">annual_package</th>
			<th scope="col">internship_requirement</th>
			...
			<tbody>
				...
				echo '<tr>';
				echo '<th scope="row">'.$row['p_offer_id'].'</th>';
				echo '<td>'.$row['reg_no'].'</td>';
				echo '<td>'.$row['comp_id'].'</td>';
				echo '<td>'.$row['offer_type'].'</td>';
			    echo '<td>'.$row['annual_package'].'</td>';
			    echo '<td>'.$row['internship_requirement'].'</td>';
